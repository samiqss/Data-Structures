...
// NOT MINIMUN SPANNING TREE YET
   ...
   for ( vert=v%n;  (vert != v-1) && num_edges < n-1; vert=(vert+1)%n) {
	...
	while (e_ptr != Adj[vert].end()) 
	{
		...
		if((vert<tester) && (set.find(tester)!=set.find(vert)))
		{
			set.merge(vert,tester);//give both vert and tester the same parent so we dont add either again to that path
			T.add_edge(vert,tester,testerw);//since they dont create a loop add the edge to T
			T.add_edge(tester,vert,testerw);//adding the edge in vector vert aswell
			Tweight=Tweight+testerw;//not sure if you want total weight of sub tree T or original tree v, currently we get back weight of T
		}
		e_ptr++;
	}
	num_edges++;
